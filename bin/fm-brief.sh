...
#   direct-PR    implement -> push + open PR via native gh (no pipeline) -> configured merge authority
...
3. Use native \`gh\` for GitHub operations and chrome-devtools-axi for browser operations.
...
When it is implemented and committed, push your branch and open a PR with \`gh pr create\`, then append \`done: PR {url}\` to the status file and stop.
