...
SH
  ...
  # number fails. This keeps the landed-work check hermetic (never reaching the
  # real gh) and represents the common "no GitHub PR" baseline. Tests that need a
  # merged PR or a lookup error override this file with the helpers below.
  cat > "$fakebin/gh" <<'SH'
#!/usr/bin/env bash
case "${1:-} ${2:-}" in
  "pr list") exit 0 ;;  # returns empty: no PR found
