...
SH
  chmod +x "$repo/bin/fm-watch-arm.sh"
  # The readiness budget bounds a successor that never reports ready, so it
  # also bounds how long the spawned login-shell arm child has to record its
  # row. Sized near cold `bash -l` start-up cost it retires an attempt before
  # the script runs and silently loses an arm row, so keep it far above that.
